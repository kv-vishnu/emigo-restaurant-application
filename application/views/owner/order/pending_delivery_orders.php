    ...
    <div class="" id="">
        <div class="popup">
                ...
                <div class="accordion-item <?php echo $isFirst; ?> ">
                    <div class="accordion-header" onclick="toggleAccordion(this)">
                        <div class="order-info">
                            ...
                            <span class="order-id">Phone : <?= $order['contact_number']; ?></span>
                            <span class="order-id">Name  : <?= $order['customer_name']; ?></span>
                            <span class="order-total"><?= round($order['total_amount'], 2); ?></span>
                        </div>
                        <span class="accordion-icon">▼</span>
                    </div>
                    <div class="accordion-body">
                        <div class="order-items delivery_orders">
                            <!-- Table Heading Row -->
                            ...
                            <?php foreach ($order['items'] as $key => $item){
                                    $productName = $this->Deliverymodel->getProductName($item['product_id']);
                                    $variantName = $this->Deliverymodel->getVariantName($item['variant_id']);
                                    $total_variant = 1;
                                    if($item['variant_id'])
                                    {
                                        $variantValue = $this->Deliverymodel->getVariantValue($item['variant_id'], $item['product_id']);
                        ...
                        <?php
                            ...
                            $approved_user = $order['approved_by']; //$order['approved_by'];
                        ...
                        <div class="action-buttons order-row">
                            <!-- <button class="btn <?= $btn_class ?>" onclick="viewOrder(1234)"><?= $btn_text ?></button> -->
                            ...
                            <button data-order-id=<?= $order['orderno']; ?> data-kot-enable=<?= $kot_enable; ?> class="btn btn-approve" <?= $approved_btn_disabled ?>>Approve</button>
                            <button data-order-id=<?= $order['orderno']; ?> class="btn btn-print print_order">Print</button>
                            <button class="btn btn-delete delete_order">Delete</button>
            ...
            <?php } else { ?>
                <div class="popup-content">
                    No pending delivery orders for this table.
                </div>
                <?php } ?>
            <!-- Popup content end -->
        </div>
